    ...
    if (field.type === 'file') return (
      <div key={field.name} className="col-span-2">
        ...
        <div {...getRootProps()} className={cn(
          ...
          <input {...getInputProps()} />
          <Upload size={16} className="mx-auto mb-1 !text-[#ffaa00]" />
          <p className="text-xs !text-[#ffaa00]" style={{ color: '#ffaa00' }}>{t('upload_text')}</p>
          <p className="text-[10px] mt-0.5 !text-[#ffaa00]" style={{ color: '#ffaa00' }}>{t('upload_formats')}</p>
        </div>
        {files.length > 0 && (
